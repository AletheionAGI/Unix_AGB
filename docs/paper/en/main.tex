...
authority. This article explains the engineering of that pipeline: contracts,
...
\begin{enumerate}[leftmargin=*]
  ...
  \item a deny-only compilation model in which learned state may restrict but
...
\section{Relation to existing security mechanisms}
...
can manufacture permission. Base Linux policy remains authoritative, and the
stateful layer can maintain or reduce existing privilege but cannot expand it.
...
\midrule
Retained-listener guardian, 256 target calls & 256/256 returned \texttt{EACCES}; decision latency 44~$\mu$s p50, 49~$\mu$s p95, 68~$\mu$s p99 \\
Concurrent broker, 256 target calls & 256/256 returned \texttt{EACCES}; 1.148~ms p50, 1.939~ms p95, 2.366~ms p99 \\
In-flight worker failure & Exact pending notification returned \texttt{EACCES} in 123~$\mu$s; replacement worker ready in 9.839~ms \\
Exact-launch VM enforcement & Protected external connect returned \texttt{EACCES}; loopback and unprotected control remained unaffected \\
...
current evidence shows that these mechanisms can be assembled and tested in a
