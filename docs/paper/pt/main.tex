...
Linux preserva a autoridade final de enforcement. Este artigo explica a
engenharia desse pipeline: contratos, identidade, ordenação, persistência,
...
\begin{enumerate}[leftmargin=*]
  ...
  \item um modelo de compilação deny-only no qual estado aprendido pode
        restringir, mas não ampliar privilégios;
...
\section{Relação com mecanismos de segurança existentes}
...
permissão. A política Linux de base conserva autoridade; a camada com estado
pode manter ou reduzir privilégios, nunca ampliá-los.
...
\midrule
Guardian com listener retido, 256 chamadas-alvo & 256/256 retornaram \texttt{EACCES}; latência de decisão de 44~$\mu$s p50, 49~$\mu$s p95 e 68~$\mu$s p99 \\
Broker concorrente, 256 chamadas-alvo & 256/256 retornaram \texttt{EACCES}; 1,148~ms p50, 1,939~ms p95 e 2,366~ms p99 \\
Falha de worker em voo & A notificação pendente exata recebeu \texttt{EACCES} em 123~$\mu$s; worker substituto pronto em 9,839~ms \\
Enforcement exact-launch em VM & Conexão externa protegida recebeu \texttt{EACCES}; loopback e controle não protegido permaneceram inalterados \\
...
durante recuperação da política. A evidência atual mostra que os mecanismos
podem ser montados e testados num protótipo Linux empacotado e opt-in. Não mostra
