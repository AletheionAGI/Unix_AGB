...
with tempfile.TemporaryDirectory(prefix="agb-fail-closed-") as directory:
    ...
    env = {**os.environ, "AGB_ADMIN_TOKEN":"fail-closed-token", "AGB_ADMIN_FAIL_CLOSED_CONFIG":"1"}
    ...
    try:
        ...
        print(json.dumps({"status":"passed","response":response,"audit_events":records},indent=2))
    finally:
