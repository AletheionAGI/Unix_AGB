...
with tempfile.TemporaryDirectory(prefix="agb-rate-limit-") as d:
 ...
 env={**os.environ,"AGB_ADMIN_TOKEN":"rate-token","AGB_ADMIN_UIDS":str(os.getuid())}
 ...
 try:
  ...
  if [r["reason"] for r in responses] != ["admin-ok"]*5+["rate-limit"] or events[-1]["reason"] != "rate-limit": raise SystemExit("rate limit behavior mismatch")
