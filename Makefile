...
gate4-prelease-teardown:
	PYTHONPATH=python:scripts python3 scripts/run_gate4_prelease_teardown.py \
		--recovery-deadline-ms "$${AGB_GATE4_RECOVERY_DEADLINE_MS:-50}" \
		--output "$${AGB_GATE4_PRELEASE_OUTPUT:-var/benchmark/gate4-prelease-teardown.json}"

gate4-control-plane-resilience:
	PYTHONPATH=python:scripts python3 scripts/run_gate4_control_plane_resilience.py \
		--output "$${AGB_GATE4_CONTROL_PLANE_OUTPUT:-var/benchmark/gate4-control-plane-resilience.json}"

gate4-transient-cgroup:
	PYTHONPATH=python:scripts python3 scripts/run_gate4_transient_cgroup.py \
		--recovery-deadline-ms "$${AGB_GATE4_RECOVERY_DEADLINE_MS:-50}" \
		--output "$${AGB_GATE4_TRANSIENT_CGROUP_OUTPUT:-var/benchmark/gate4-transient-cgroup.json}"

gate4-package:
	python3 scripts/build_gate4_deb.py \
		--output-dir "$${AGB_GATE4_PACKAGE_DIR:-var/package}"
...
evaluate-gate4-promotion:
	PYTHONPATH=python:scripts python3 scripts/evaluate_gate4_promotion.py \
		--evidence "$${AGB_GATE4_PROMOTION_EVIDENCE:-var/benchmark/gate4-promotion-evidence.jsonl}" \
		--policy-revision "$${AGB_GATE4_POLICY_REVISION:?set AGB_GATE4_POLICY_REVISION}" \
		--output "$${AGB_GATE4_PROMOTION_OUTPUT:-var/benchmark/gate4-promotion-summary.json}"
...
gate4-formal-vm-prepare:
	sudo python3 scripts/prepare_gate4_formal_vm.py --apply

gate4-formal-freeze:
	python3 scripts/build_gate4_formal_manifest.py \
		--profile fixtures/benchmark/gate4-campaign-formal-profile.json \
		--output "$${AGB_GATE4_FORMAL_MANIFEST:-var/benchmark/gate4-campaign-formal-manifest.json}"

gate4-formal:
	python3 scripts/run_gate4_automated_campaign.py \
		--manifest "$${AGB_GATE4_FORMAL_MANIFEST:-var/benchmark/gate4-campaign-formal-manifest.json}" \
		--mode formal --duration-seconds 28800 --interval-seconds 5 \
		--output-dir "$${AGB_GATE4_FORMAL_OUTPUT:-var/benchmark/gate4-campaign-formal}"
...
gate4-formal-gui-view:
	python3 scripts/serve_gate4_campaign_gui.py \
		--output-dir "$${AGB_GATE4_FORMAL_OUTPUT:-var/benchmark/gate4-campaign-formal}" \
		--port "$${AGB_GATE4_CAMPAIGN_GUI_PORT:-8765}"
...
gate4-formal-evaluate:
	python3 scripts/evaluate_gate4_formal_campaign.py \
		--profile fixtures/benchmark/gate4-campaign-formal-profile.json \
		--manifest "$${AGB_GATE4_FORMAL_MANIFEST:-var/benchmark/gate4-campaign-formal-manifest.json}" \
		--output-dir "$${AGB_GATE4_FORMAL_OUTPUT:-var/benchmark/gate4-campaign-formal}" \
		--vm-evidence "$${AGB_GATE4_VM_EVIDENCE:-var/benchmark/gate4-vm-evidence.json}" \
		--namespace-evidence "$${AGB_GATE4_NAMESPACE_EVIDENCE:-var/benchmark/gate4-namespace-evidence.json}" \
		--resource-evidence "$${AGB_GATE4_RESOURCE_EVIDENCE:-var/benchmark/gate4-resource-evidence.json}" \
		--output "$${AGB_GATE4_FORMAL_EVALUATION:-var/benchmark/gate4-formal-evaluation.json}"
